        ...
        if ($bluem_config->sequenceType === 'OOFF' || sizeof($db_results) === 0)
        {
            ...
            try {
                ...
                if ( ! isset( $response->EMandateTransactionResponse->TransactionURL ) ) {
                    $msg = esc_html__("Er ging iets mis bij het aanmaken van de transactie.<br>
...
function bluem_mandates_instant_callback()
{
    ...
    bluem_error_report_email(
        [
            ...
            'message'  => sprintf(
                /* translators: %s: status code */
                esc_html__("Fout: Onbekende of foutieve status teruggekregen: %s<br>Neem contact op met de webshop en vermeld deze status; gebruiker wel doorverwezen terug naar site",'bluem'), $statusCode)
        ]
    );
    wp_redirect( $bluem_config->instantMandatesResponseURI . "?result=false&reason=error" );
    exit;

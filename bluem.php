...
function bluem_woocommerce_no_woocommerce_notice() {
	if ( is_admin() ) {
		...
		if ( ! isset( $bluem_options['suppress_woo'] ) || $bluem_options['suppress_woo'] === "0" ) {
			...
			printf( wp_kses_post( '&nbsp;<strong>De Bluem integratie is grotendeels afhankelijk van WooCommerce - installeer en/of activeer deze plug-in.</strong><br/>
...
function bluem_woocommerce_no_permalinks_notice() {
	if ( is_admin() ) {
		...
		esc_html_e( "De Bluem integratie is vanwege de routing afhankelijk van de WordPress Permalink instelling.", 'bluem' );
		echo "</strong><br>";
		esc_html_e( "Selecteer een optie BEHALVE 'Eenvoudig' bij de Permalink in de ", 'bluem' );
		echo '<a href="' . esc_url( admin_url( 'options-permalink.php' ) ) . '" target="_blank">' . esc_html__( 'Instellingen', 'bluem' ) . '</a></p>
        </div>';
	}
}

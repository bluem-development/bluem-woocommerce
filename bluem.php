...
const BLUEM_PLUGIN_VERSION = '1.5.2';
...
function bluem_update_request_by_id( $request_id ) {
    ...
    if ( count( $request_query ) == 0 ) {
        bluem_requests_view_with_filter( [] );
...
function bluem_requests_view_type( $type = 'identity' ): void {
    if ( ! in_array( $type, BLUEM_TRANSACTION_REQUEST_TYPES, true ) ) {
        echo 'Invalid request type';

        return;
    }

    bluem_requests_view_with_filter( [ $type ] );
}

function bluem_requests_view_with_filter( $filters = [], string $current_category = '' ): void {

    $requests = bluem_get_requests_per_type( $filters );

    $users_by_id = bluem_get_users_by_id();
...
function bluem_get_requests_per_type( $filters = [] ): array {
    ...
    $_requests = ! empty( $params )
            ? $wpdb->get_results( $wpdb->prepare( $query, ...$params ) )
            : $wpdb->get_results( $query );

    return bluem_sort_requests_per_type( $_requests );
...
function bluem_setup_incomplete() {
    ...
    if ( $options == false ) {
        ...
        if ( bluem_module_enabled( 'payments' ) ) {
            // Payment BrandIDs may be custom values, so only require them to be configured.
